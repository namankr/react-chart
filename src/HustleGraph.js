...
const Graph1 = (d) => {
  ...
  const CustomLabel = ({ viewBox, d }) => {
    const { cx, cy } = viewBox;
    const { mentalEnergyBaseLinePercentage, mentalEnergyPercentage } = d.data;
    let percentageAboveBaseLine = null;
    let percentageBelowBaseLine = null;
    let atBaseLine = null;
    if (mentalEnergyPercentage > mentalEnergyBaseLinePercentage) {
      percentageAboveBaseLine = Math.round(
        (mentalEnergyPercentage / mentalEnergyBaseLinePercentage) * 100
      );
    } else if (mentalEnergyPercentage === mentalEnergyBaseLinePercentage) {
